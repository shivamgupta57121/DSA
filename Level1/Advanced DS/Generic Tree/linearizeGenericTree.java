...
public class Main {
  ...
  public static void linearize(Node node){
    ...
    // to be done in postorder
    // faith that all child will be linearized
    for(Node child : node.children ){
        linearize(child);
    }
    
    // own work to linearize
    while(node.children.size() > 1){
        Node lc = node.children.remove(node.children.size()-1); // remove last child from tree
        Node slc = node.children.get(node.children.size()-1); // get second last child
        
        Node tail = getTailNode(slc); // get tail node of second last child where we need to add last child
        tail.children.add(lc); // add last child
    }
